		...
		<div class="container">
			<div class="row">
      		...
      		<div class="col-md-12 whiteBox">
                  ...
                  <form id="filterForm" method="POST" action="${pageContext.request.contextPath}/filterForScheduleListSite.htm">
                  <div class="col-md-12 noPadding">
								 ...
								 <table id="scheduleTable" style="font-size: 12px;">
									...
									<tbody>
										<c:forEach var="tourDetails" items="${tournamentList}" varStatus ="loop"> 										 
											<tr>
											 <td style="text-align: center !important;">${loop.count}</td>
                    						<td class="alignCenter">${tourDetails.tournamentName}</td>
												<%-- <td>${loop.count}</td>
											    ...
											    <td class="alignCenter"><a href="${pageContext.request.contextPath}/${tourDetails.homeTeamName}/board/${tourDetails.homeTeamId}"><a href="javascript:void(0);" onclick="PleaseLogin()">${tourDetails.homeTeamName}</a></a></td>
												<td><a href="${pageContext.request.contextPath}/${tourDetails.awayTeamName}/board/${tourDetails.awayTeamId}"><a href="javascript:void(0);" onclick="PleaseLogin()">${tourDetails.awayTeamName}</a></a></td>
...
function loadPrev(text){
	...
	$.ajax({	
		...
		success: function(response){
			...
			document.getElementById("hiddenDateStart").value = response.startDateStr;			document.getElementById("hiddenDate").value = response.endDateStr;			
			/* if(text =='previous'){
			...
			if(res.length != 0){
				...
				//	 $("#hiddenDate").val(mavValue);
					...
					html += '<td><a href="${pageContext.request.contextPath}/'+res[i].homeTeamName+'/board/'+res[i].homeTeamId+'"><a href="javascript:void(0);" onclick="PleaseLogin()">'+res[i].homeTeamName+'</a></a></td>';
					html += '<td><a href="${pageContext.request.contextPath}/'+res[i].awayTeamName+'/board/'+res[i].awayTeamId+'"><a href="javascript:void(0);" onclick="PleaseLogin()">'+res[i].awayTeamName+'</a></a></td>';
					...
					for(var j=0; j<umpireList.length; j++){
						html += '<span>'+'<a href="${pageContext.request.contextPath}/buddy/'+umpireList[j].umpireName+'/'+umpireList[j].umpireId+'"><a href="javascript:void(0);" onclick="PleaseLogin()">'+umpireList[j].umpireName+'</a></a>';
					...
					for(var k=0; k<scorerList.length; k++){
						html += '<span>'+'<a href="${pageContext.request.contextPath}/buddy/'+scorerList[k].scorerName+'/'+scorerList[k].scorerId+'"><a href="javascript:void(0);" onclick="PleaseLogin()">'+scorerList[k].scorerName+'</a></a>';
